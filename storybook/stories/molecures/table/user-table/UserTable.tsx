    ...
    <table className="userTable">
      <tr className="userTable-row">
        <th className="userTable-headerElement">
          <input type="checkbox" className="checkbox-input checkbox-sm" />
        </th>
      ...
      {users.map((user) => (
        <tr
          key={user.id}
          className={`userTable-row${user.id === removingId ? ' removing' : ''}${selectedIds.has(user.id) ? ' userTable-selectedRow' : ''}`}
        >
          <td className="userTable-rowElement">
            ...
            <input
              ...
              onChange={(e) =>
                handleRowSelectionChange(user.id, e.target.checked)
              }
            />
          </td>
