...
export const BuildPagination = ({
  ...
  const validatedInitialPage = pageRange.includes(initialSelectedPage)
    ? initialSelectedPage
    : Math.ceil(pageRange[pageRange.length - 1] / 2);
  ...
  return (
    <div className="pagination">
      <div
        className={`iconButton-md ${checkButtonDisabled('first') ? '  button-disabled' : `${buttonStyle.length ? ` button-${buttonStyle} button-ghost` : ''}`}`}
        onClick={() => {
      ...
      <div
        className={`iconButton-md ${checkButtonDisabled('previous') ? '  button-disabled' : `${buttonStyle.length ? ` button-${buttonStyle} button-ghost` : ''}`}`}
        onClick={() => {
      ...
      {pageRange.map((pageNumber) => {
        return (
          <div
            ...
            onClick={() =>
              pageNumber === currentPage ? null : setCurrentPage(pageNumber)
            }
          >
            {pageNumber}
          </div>
      ...
      <div
        className={`iconButton-md ${checkButtonDisabled('next') ? '  button-disabled' : `${buttonStyle.length ? ` button-${buttonStyle} button-ghost` : ''}`}`}
        onClick={() => {
      ...
      <div
        className={`iconButton-md ${checkButtonDisabled('last') ? '  button-disabled' : `${buttonStyle.length ? ` button-${buttonStyle} button-ghost` : ''}`}`}
        onClick={() => {
